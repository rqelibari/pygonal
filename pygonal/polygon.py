...
class Polygon(pygonal.Seq2):
    ...
    :param vertices: Iterable containing three or more :class:`~pygonal.Vec2`
        objects.
    ...
    def regular(cls, vertex_count, radius, center=(0, 0), angle=0):
        ...
        poly._centroid = pygonal.Vec2(*center)
        poly._max_r = radius
    ...
    def bounding_box(self):
        ...
        if self._bbox is None:
            self._bbox = pygonal.BoundingBox(self)
        return self._bbox
    ...
    def _classify(self):
        ...
        for delta in itertools.ifilter(
            ...
            if cross > 0.0: # XXX Should this be cross > pygonal.EPSILON?
                if angle_sign == -1:
                    self._convex = False
                    break
                angle_sign = 1
    ...
    def tangents_to_point(self, point):
        ...
        :rtype: tuple of :class:`~pygonal.Vec2`
        """
...
def _ahull_partition_points(hull, points, p0, p1):
    ...
    for p in points:
        ...
        if dist > furthest:
            furthest = dist
            partition_point = p
    partition_point = pygonal.Vec2(*partition_point)
