...
Polygons in ``pygonal`` are represented as a sequence of vertices. Each pair of
vertices defines one edge of the polygon. The simplest polygon is the
triangle, thus a :class:`pygonal.Polygon` must have at least three vertices.
...
resulting polygon in whatever order they are supplied::
	...
	>>> from pygonal import Polygon
	>>> poly = Polygon([(0,-1), (2,0), (0,1), (-2,0)])

It is also easy to construct regular polygons. To do so, you supply the
:meth:`~pygonal.Polygon.regular` method with the number or vertices, radius
...
The :meth:`~pygonal.Polygon.star` method lets you create radial polygons with
...
The first argument of :meth:`~pygonal.Polygon.star` is the number of "peaks".
...
non-convex. ``pygonal`` can compute the classification for any arbitrary
...
boundaries. The :attr:`~pygonal.Polygon.is_simple` attribute of a polygon
...
are also simple. The :attr:`~pygonal.Polygon.is_convex` polygon attribute can
...
Many operations have very fast algorithms for convex polygons, so ``pygonal``
...
doubt, let ``pygonal`` determine these values for you.

Other Attributes
----------------

As mentioned above, simple polygons have a centroid point. This is accessible
via the :attr:`~pygonal.Polygon.centroid` attribute. For non-simple polygons
...
:attr:`~pygonal.Polygon.bounding_box` attribute. This is the smallest
:class:`~pygonal.BoundingBox` that completely encloses the polygon.
...
polygon. To facilitate this, the :class:`~pygonal.Polygon` class provides
an alternate constructor method, :meth:`~pygonal.Polygon.convex_hull`.

The :meth:`~pygonal.Polygon.convex_hull` method accepts any sequence of
...
:class:`~pygonal.Affine` objects. Using infix multiplication, this results in a
...
:meth:`~pygonal.Polygon.contains_point` method. This method works regardless of
the classification of the polygon, and is not sensitive to the winding
direction. :meth:`~pygonal.Polygon.contains_point` is highly efficient for
...
:meth:`~pygonal.Polygon.tangents_to_point` method. This works for any arbitrary
